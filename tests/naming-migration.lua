-- The naming migration (docs/MEMORY.md) against a database built the OLD way.
--
...
check(not exists("messages_conversation_idx")
    and one("SELECT tbl_name FROM sqlite_master WHERE type='index' AND name='ledger_messages_conversation_idx'").tbl_name == "ledger_messages"
    and one("SELECT tbl_name FROM sqlite_master WHERE type='index' AND name='ledger_messages_time_idx'").tbl_name == "ledger_messages",
  "the ledger's indexes now belong to the ledger")
...
check(exists("messages_session_idx") and not exists("turns_session_idx")
    and one("SELECT tbl_name FROM sqlite_master WHERE type='index' AND name='messages_time_idx'").tbl_name == "messages",
