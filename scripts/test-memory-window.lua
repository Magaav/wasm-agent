...
ok(memory.turn_count(id) == 260, "the fixture must have 260 turns", memory.turn_count(id))

-- The default window is the newest 200, in order, starting 60 turns into the thread.
local rows = memory.session_turns(id)
ok(#rows == 200, "the default window is 200 turns", #rows)
...
local after = memory.session_turns(id, { after_seq = 250 })
ok(#after == 10 and tonumber(after[1].seq) == 251 and tonumber(after[#after].seq) == 260,
  "after_seq must return the newest turns of the span", tostring(#after))

-- An explicit limit is a choice, not a ceiling.
local everything = memory.session_turns(id, { limit = 1000 })
...
ok(type(result) == "table" and type(result.turns) == "table", "the session tool must return turns")
ok(#result.turns == 200, "the tool window is 200 turns", #result.turns)
ok(type(result.note) == "string" and result.note:find("newest 200 of 260", 1, true),
  "the tool must say what it dropped", result.note)
ok(tonumber(result.turns[1].seq) == 61, "the tool must show the newest turns, not the oldest",
  result.turns[1].seq)
...
ok(#short.turns == 5, "a short thread must come back whole", #short.turns)
