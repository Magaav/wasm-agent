...
/// shell call is bounded and a killed command says so.
///
/// The runner is `wa_operation`, the one owner of subprocess lifetime: a second
/// shell runner with `Command::output`, `read_to_end` or detached reader threads is
/// how a run gets held open by an inherited pipe (docs/HOST.md). The caller's
/// patience still travels with the command, so a timeout means the work stopped
/// rather than that the caller stopped waiting.
fn shell(args: &Value, budget_ms: u64) -> Value {
    ...
    // An explicit timeout_ms, otherwise the caller's budget minus a margin, bounded
    // again by the node-wide exec deadline. The operation is killed when it expires.
    let configured = std::env::var("WASM_AGENT_EXEC_TIMEOUT_SECONDS")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .unwrap_or(300);
    let budget = args["timeout_ms"]
        .as_u64()
        .unwrap_or(budget_ms.saturating_sub(2_000))
        .clamp(1_000, 290_000);
    let seconds = configured.min(budget.saturating_add(999) / 1000).max(1);
    ...
    match manager.start(spec) {
        Ok(id) => match manager.wait(&id, std::time::Duration::from_secs(seconds.saturating_add(2))) {
            ...
            Ok(_) => {
                ...
                json!({"operation_id":id,"ok":false,"error":"operation_supervisor_overdue","cleanup":"unknown"})
            }
