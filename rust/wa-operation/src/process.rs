...
impl Process {
    pub fn spawn(spec: &Spec) -> io::Result<Self> {
        ...
        command.args(&spec.args).stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::piped()).process_group(0);
        if !spec.cwd.is_empty() { command.current_dir(&spec.cwd); }
        for (k,v) in &spec.env { command.env(k,v); }
        let mut child = command.spawn()?;
        let stdout = child.stdout.take();
        let stderr = child.stderr.take();
        let result = Self {group: child.id() as i32, child, stdout, stderr};
        for fd in [result.stdout.as_ref().unwrap().as_raw_fd(), result.stderr.as_ref().unwrap().as_raw_fd()] {
    ...
    pub fn containment() -> &'static str { "posix_process_group" }
    pub fn read(&mut self, stderr: bool, buffer: &mut [u8]) -> io::Result<Option<usize>> {
        use std::io::Read;
        let result = if stderr {
            match self.stderr.as_mut() {Some(p)=>p.read(buffer),None=>return Ok(Some(0))}
        } else {
            match self.stdout.as_mut() {Some(p)=>p.read(buffer),None=>return Ok(Some(0))}
        };
        match result {
