// Adapted from candle-transformers 0.11.0 src/models/whisper/audio.rs
// (MIT OR Apache-2.0), itself adapted from whisper.cpp.
// The frame loop runs serially because wasm-agent's plugin host supplies no
// thread imports. The original Candle worker spawned native threads.
...
pub fn log_mel_spectrogram_<T: Float>(
    ...
    // Wasmtime's no-import plugin runtime has no threads. Compute each frame
    // in this instance rather than calling std::thread::scope.
...
pub fn pcm_to_mel<T: Float>(cfg: &m::Config, samples: &[T], filters: &[T]) -> Vec<T> {
